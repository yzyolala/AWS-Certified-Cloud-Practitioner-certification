...
IAM（Identity and Access Management）是AWS提供的一个免费服务，允许用户管理对AWS服务和资源的访问。VPC（Virtual Private Cloud）允许用户在AWS云中启动资源，如Amazon EC2实例，这在一定限制内也是免费的。Consolidated Billing是AWS账户之间的联合账单功能，它本身是免费的。至于Elastic Beanstalk，它是一个免费使用的服务，但在其上运行的应用程序可能会产生费用，因为它会使用其他AWS资源，如EC2实例和S3存储，这些可能会超出免费使用层次。
