...
| 主要功能和特点              | - 具有HTTP路由功能                           | - 高性能，每秒可处理**数百万请求**     | - 在IP数据包层级上进行流量路由              |
...
自动增加或减少 EC2 实例的数量。这样，在应用程序负载增加时，它可以自动添加更多的实例，而在负载减少时，可以自动删除多余的实例。

## ASG跟ELB(Loadbalancer)区别

ASG 管理实例的数量，而 Load Balancer 处理流量的分发和负载均衡。结合使用这两个服务可以构建强大和高可用的应用程序架构。
...
目的是**确保应用程序即使在出现故障或灾难情况时也能继续正常运行**。高可用性通常涉及冗余和故障转移机制来最小化单点故障的风险。
